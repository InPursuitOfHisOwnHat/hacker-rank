...
// === ANSWER (3 functions = sum, min, max)
int  sum (int count,...) {
    va_list(nums);
    va_start(nums, count);
    int sum=0;
...
int min(int count,...) {
    ...
    va_list(nums);
    va_start(nums, count);
    int curr_min = va_arg(nums, int);
    int curr_number;
    for( int i=1; i<count; i++) {
        curr_number = va_arg(nums, int);
        //printf("Curr min: %d, Curr num: %d\n", curr_min, curr_number);
        if( curr_number < curr_min ) {
            curr_min = curr_number;
        }
    }
    va_end(nums);
